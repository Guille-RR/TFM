...
def paralellize_state_processing(q, unitaries, ansatz_sep, sampler, backend):
    ...
    res = minimize(cost_function, x0, args=(combined_qc, sampler, backend, history, q), method='COBYLA', tol=1e-10)
...
def main():
    ...
    states_to_classify = 500
    optimizer = 'DE'

    ansatz_sep = QuantumCircuit(2)
    params = ParameterVector('θ', 6)
    ansatz_sep.rx(params[0], 0)
    ansatz_sep.ry(params[1], 0)
    ...
    if optimizer == 'COBYLA':
        results = Parallel(n_jobs=-1)(
            delayed(paralellize_state_processing)(q, unitaries, ansatz_sep, sampler, backend)
            for q in range(states_to_classify)
        )
    elif optimizer == 'DE':
        results = Parallel(n_jobs=-1)(delayed(de_state_processing)(q, unitaries, ansatz_sep, sampler, backend)
            for q in range(states_to_classify))
    else:
        raise ValueError("Unsupported optimizer. Use 'COBYLA' or 'DE'.")
    dictionary = {}
    computed_fidelity = []
    for q, state_type, final_fid, history in results:
        state_key = f"state{q}"
        ...
        computed_fidelity.append(final_fid)

    dictionary['results'] = computed_fidelity
    dictionary['time'] = time.time() - time1
    if not os.path.exists('2_qubit/'):
        os.makedirs('2_qubit/')
    else:
        print("Directory already exists")
    with open(f'2_qubit/de_{states_to_classify}_states.json', 'w') as f:
